function MATS = MatAssem2D_MS2()
    ...
    I24 = Integral(Omega,L4.val*dt.val*((4*sqrt(3)/3)*(-q_bar.grad(1,2)*uu.grad(1,2) + q_bar.grad(1,1)*uu.grad(1,1)) + 2*q_bar.grad(2,2)*uu.grad(1,2))*vv.val(1));
    I25 = Integral(Omega,L4.val*dt.val*(2*q_bar.grad(1,2)*uu.grad(2,2))*vv.val(1));
    I26 = Integral(Omega,L4.val*dt.val*((2*sqrt(3)/3)*(2*q_bar.grad(2,1)*uu.grad(1,1) - q_bar.grad(2,2)*uu.grad(1,2))*vv.val(2)));
    I27 = Integral(Omega,L4.val*dt.val*((2*sqrt(3)/3)*(2*q_bar.grad(1,1)*uu.grad(2,1) - q_bar.grad(1,2)*uu.grad(2,2)) + 4*q_bar.grad(2,2)*uu.grad(2,2))*vv.val(2));
    Mass_Matrix2 = Mass_Matrix2 + I1 + I2 + I3 + I4 + I12 + I13 + I24 + I25 + I26 + I27;
    ...
    I28 = Integral(Omega,L4.val*dt.val*((2*sqrt(3)/3)*(4*q_bar.grad(1,1)*uu.val(1)*vv.grad(1,1) + 4*q_bar.val(1)*uu.grad(1,1)*vv.grad(1,1) - 2*q_bar.grad(1,2)*uu.val(1)*vv.grad(1,2) - 2*q_bar.val(1)*uu.grad(1,2)*vv.grad(1,2) - q_bar.val(2)*uu.grad(1,2)*vv.grad(1,2)) + 2*q_bar.grad(2,2)*uu.val(1)*vv.grad(1,2)));
    I29 = Integral(Omega,L4.val*dt.val*((2*sqrt(3)/3)*(-q_bar.grad(2,2)*uu.val(2)*vv.grad(1,2) + 2*q_bar.grad(2,1)*uu.val(2)*vv.grad(1,1) + 2*q_bar.val(2)*uu.grad(2,1)*vv.grad(1,1)) + 2*q_bar.val(1)*uu.grad(2,2)*vv.grad(1,2)));
    I30 = Integral(Omega,L4.val*dt.val*((4*sqrt(3)/3)*q_bar.val(2)*uu.grad(1,1)*vv.grad(2,1) + 2*q_bar.val(1)*uu.grad(1,2)*vv.grad(2,2) + 2*q_bar.grad(1,2)*uu.val(1)*vv.grad(2,2)));
    I31 = Integral(Omega,L4.val*dt.val*((2*sqrt(3)/3)*(-q_bar.val(2)*uu.grad(1,2)*vv.grad(2,2) - q_bar.grad(1,2)*uu.val(2)*vv.grad(2,2) + 2*q_bar.grad(1,1)*uu.val(2)*vv.grad(2,1)) + 4*q_bar.grad(2,2)*uu.val(2)*vv.grad(2,2) + 4*q_bar.val(2)*uu.grad(2,2)*vv.grad(2,2)));
    Stiff_Matrix2 = Stiff_Matrix2 + I14 + I15 + I16 + I17 + I28 + I29 + I30 + I31;
    ...
    I34 = Integral(Omega,-LS.val*dt.val*((6*q_bar.grad(1)' + 6*sqrt(3)*q_bar.grad(2)') * vv.grad(1) + (18*q_bar.grad(2)' + 6*sqrt(3)*q_bar.grad(2)') * vv.grad(2)));
    ...
    I32 = Integral(Omega,L4.val*dt.val*((2*sqrt(3)/3)*(q_bar.grad(1,2)*q_bar.grad(1,2)*vv.val(1) - 2*q_bar.grad(1,1)*q_bar.grad(1,1)*vv.val(1) - 4*q_bar.val(1)*q_bar.grad(1,1)*vv.grad(1,1) - 2*q_bar.val(2)*q_bar.grad(2,1)*vv.grad(1,1) + 2*q_bar.val(1)*q_bar.grad(1,2)*vv.grad(1,2) + q_bar.val(2)*q_bar.grad(1,2)*vv.grad(1,2)) - 2*q_bar.grad(1,1)*q_n.grad(3,2)*vv.val(1) - 2*q_bar.grad(1,2)*q_bar.grad(2,2)*vv.val(1) - 2*q_bar.grad(1,2)*q_n.grad(3,1)*vv.val(1) - 2*q_bar.val(1)*q_n.grad(3,2)*vv.grad(1,1) - 2*q_bar.val(1)*q_bar.grad(2,2)*vv.grad(1,2)));
    I33 = Integral(Omega,L4.val*dt.val*((2*sqrt(3)/3)*(-2*q_bar.grad(1,1)*q_bar.grad(2,1)*vv.val(2) + q_bar.grad(1,2)*q_bar.grad(2,2)*vv.val(2) - 2*q_bar.val(2)*q_bar.grad(1,1)*vv.grad(2,1) + q_bar.val(2)*q_bar.grad(1,2)*vv.grad(2,2)) - 2*q_bar.grad(2,1)*q_n.grad(3,2)*vv.val(2) - 2*q_bar.grad(2,2)*q_bar.grad(2,2)*vv.val(2) - 2*q_bar.grad(2,2)*q_n.grad(2,1)*vv.val(2) - 2*q_bar.val(2)*q_n.grad(3,2)*vv.grad(2,1) - 2*q_bar.val(1)*q_bar.grad(1,2)*vv.grad(2,2) - 4*q_bar.val(2)*q_bar.grad(2,2)*vv.grad(2,2) - 2*q_bar.val(2)*q_n.grad(3,1)*vv.grad(2,2)));
    RHS2 = RHS2 + I5 + I6 + I7 + I8 + I9 + I10 + I11 + I18 + I19 + I20 + I21 + I22 + I23 + I32 + I33 + I34;
